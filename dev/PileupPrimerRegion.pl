...
my $alnID = Database->new($alnDB)->lookup(1,$bam_rm_unique,0);
defined $alnID or die "[STDERR]: alnID for '$bam_rm_unique' not defined in '$alnDB'\n";
################################################################################

### LOAD BAM & INDEX ###########################################################
-e $HOME . "/" . $bam && -e $index or die "[STDERR]: 'bam' $bam or 'index' $index don't exist\n";
($bam =~ /unique/ or die "[STDERR]: specified unique_aln_only but bam !~ /unique/\n") if $unique_aln_only;
my $BAM = Bio::DB::Sam->new(-bam => $HOME . "/" . $bam,-fasta => $index,-autoindex => 1,-split_splices => 1);
################################################################################
...
### OPEN OUTPUT FILE ###########################################################
my ($bam_name,$bam_dir,$bam_exit) = fileparse($HOME."/".$bam,'\.bam');
$bam_dir =~ s/aln/rdd/;
$bam_name =~ s/\.rmspl//;
	...
	foreach my $STRANDS (@strands) {
		...
		foreach my $ADAPTER_ONLY (split(',',$adapter_only)) {
			foreach my $UNIQUE_SEQ_ONLY (split(',',$unique_seq_only)) {
				...
				### STATS FOR ALL BASES
				my ($COUNT,$QUAL,$MAPQUAL,$POS5,$POS3) = get_RDD_stats($RDD_OBJ,$TOTAL_BASES,$STRANDS,$ADAPTER_ONLY,$UNIQUE_SEQ_ONLY);
		
				### STATS FOR RDD BASE
				my ($RDD_COUNT,$RDD_QUAL,$RDD_MAPQUAL,$RDD_POS5,$RDD_POS3) = get_RDD_stats($RDD_OBJ,[$RDD_BASE_new],$STRANDS,$ADAPTER_ONLY,$UNIQUE_SEQ_ONLY);
