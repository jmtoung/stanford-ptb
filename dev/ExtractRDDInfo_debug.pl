...
my $alnID = "ignore"; #Database->new($alnDB)->lookup(1,$bam_rm_unique,0);
#defined $alnID or die "[STDERR]: alnID for '$bam_rm_unique' not defined in '$alnDB'\n";
################################################################################

### LOAD BAM & INDEX ###########################################################
-e $bam && -e $index or die "[STDERR]: 'bam' $bam or 'index' $index don't exist\n";
($bam =~ /unique/ or die "[STDERR]: specified unique_aln_only but bam !~ /unique/\n") if $unique_aln_only;
my $BAM = Bio::DB::Sam->new(-bam => $bam,-fasta => $index,-autoindex => 1,-split_splices => 1);
################################################################################
...
### OPEN OUTPUT FILE ###########################################################
my ($bam_name,$bam_dir,$bam_exit) = fileparse($bam,'\.bam');
$bam_dir =~ s/aln/rdd/;
$bam_name =~ s/\.rmspl//;
