...
my $pruneIn = "/srv/gsfs0/projects/butte/jmtoung/PTB-Genetics/Data/allG1G2-1000G-HRS/step3-PCA/step4-OmniDataFromSuyash-Phase3/1000G-as-controls/exclude-CG-AT-snps/exclude-141/20160602_NADAV/step12-LD/all_merged_mode_5_filtered_ldpt2.prune.in";

my ($bname,$bdir,$bext) = fileparse($bfile,'');

my $out = $bname . "_ld";

my $command = "$plink --bfile $bfile --extract $pruneIn --recode --make-bed --out $out";
